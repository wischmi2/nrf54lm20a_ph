...
static psa_key_id_t import_raw_pk(const uint8_t *private_key, size_t size)
{
    ...
    if (err) {
        LOG_ERR("Failed to parse key: -0x%x", (unsigned int)-err);
    ...
    if (err) {
        LOG_ERR("Failed to import private key: -0x%x", (unsigned int)-err);
        return PSA_KEY_ID_NULL;
    }
...
static ssize_t read_file(const char *path, uint8_t **out)
{
    ...
    size = fs_read(&file, buf, (size_t)size);
    if (size < 0) {
        LOG_ERR("Could not read %s, err: %d", path, (int)size);
        free(buf);
        goto finish;
    }

    LOG_DBG("Read %d bytes from %s", (int)size, path);
    *out = buf;
...
psa_key_id_t load_private_key(void)
{
    ...
    psa_key_id_t key_id = import_raw_pk(buf, (size_t)size);
    free(buf);
    return key_id;
}
...
int load_certificate(struct pouch_cert *cert)
{
    ...
    cert->buffer = buf;
    cert->size = (size_t)size;
    LOG_DBG("Read certificate (%d bytes)", (int)size);
    return 0;
}
